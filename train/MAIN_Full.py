...
from utils import *
from models.TMF_PINN import SVE
...
for i in range(6):
    ...
    u_star = u_exact.flatten()[:, None]  # 展平
    ...
    u_BC = np.vstack([uu2, uu3])  # 流速用的FVM数据
    h_BC = np.vstack([hup[i], hdown[i]])  # 压力用的节点数据
    ...
    for iobs in ind_obs_u:
        ...
        if np.isnan(add_noise(u_exact[:Nt_train, iobs])).any:
    ...
    for iobs in ind_obs_h:
        ...
        if np.isnan(add_noise(h_exact[:Nt_train, iobs])).any:
    ...
    ## saving data
    name = f'results/PINN_full/pipe{i+1}/'
