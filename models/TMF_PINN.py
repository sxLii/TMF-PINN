...
class SVE:
    ...
    def __init__(self, X_h_IC, 
                      ...
                      lr=5e-4,ac_bcs_h=1.0,ac_bcs_u=1.0,
                      ac_ics_h=1.0, ac_obs_u=1.0,ac_obs_h=1.0,
        ...
        self.u_star = u_star
        self.h_star = h_star
        self.alpha_star = tf.zeros_like(h_star)

        # Adaptive re-weighting constant
        self.beta = 0.9
        self.adaptive_constant_bcs_u_val = np.array(ac_bcs_u)
        self.adaptive_constant_bcs_h_val = np.array(ac_bcs_h)
        self.adaptive_constant_ics_h_val = np.array(ac_ics_h)
        self.adaptive_constant_obs_u_val = np.array(ac_obs_u)
        self.adaptive_constant_obs_h_val = np.array(ac_obs_h)
        ...
        # layers first layer is map to fourier space
        layers1=layers1[1:]
        layers2=layers2[1:]
        ...
        self.Wh1_t = tf.Variable(tf.random_normal([1, layers1[0] // 2], dtype=self.DTYPE) * 1.0,
                                dtype=self.DTYPE, trainable=False)
        self.Wh2_t = tf.Variable(tf.random_normal([1, layers1[0] // 2], dtype=self.DTYPE) * 10,
                                dtype=self.DTYPE, trainable=False)
        self.Wh1_x = tf.Variable(tf.random_normal([1, layers1[0] // 2], dtype=self.DTYPE) * 1.0,
                                dtype=self.DTYPE, trainable=False)

        self.Wu1_t = tf.Variable(tf.random_normal([1, layers2[0] // 2], dtype=self.DTYPE) * 1.0,
                                 dtype=self.DTYPE, trainable=False)
        self.Wu2_t = tf.Variable(tf.random_normal([1, layers2[0] // 2], dtype=self.DTYPE) * 10,
                                 dtype=self.DTYPE, trainable=False)
        self.Wu1_x = tf.Variable(tf.random_normal([1, layers2[0] // 2], dtype=self.DTYPE) * 1.0,
                                 dtype=self.DTYPE, trainable=False)
        ...
        self.dummy_tf1 = tf.placeholder(self.DTYPE, shape=(None, layers1[-1])) # dummy variable for fwd_gradients
        ...
        self.loss = self.loss_f + self.loss_BCs + self.loss_ICs #+self.loss_constant
        ...
        for i in range(len(self.layers1) - 1):
            self.grad_res_h.append(tf.gradients(self.loss_f, self.weights1[i])[0])
            self.grad_bcs_h.append(tf.gradients(self.loss_BC_h, self.weights1[i])[0])
            self.grad_ics_h.append(tf.gradients(self.loss_IC_h, self.weights1[i])[0])
            if self.useObs:
                self.grad_obs_h.append(tf.gradients(self.loss_obs_h, self.weights1[i])[0])

        for i in range(len(self.layers2) - 1):
            self.grad_res_u.append(tf.gradients(self.loss_f, self.weights2[i])[0])
            self.grad_bcs_u.append(tf.gradients(self.loss_BC_u, self.weights2[i])[0])
            if self.useObs:
                self.grad_obs_u.append(tf.gradients(self.loss_obs_u, self.weights2[i])[0])


        ## adpative_constant_bcs_u_list
        ...
        for i in range(len(layers1) - 1):
            self.adpative_constant_bcs_h_list.append(
                tf.reduce_max(tf.abs(self.grad_res_h[i])) / tf.reduce_mean(tf.abs(self.grad_bcs_h[i])))
            self.adpative_constant_ics_h_list.append(
                tf.reduce_max(tf.abs(self.grad_res_h[i])) / tf.reduce_mean(tf.abs(self.grad_ics_h[i])))
            if self.useObs:
                self.adpative_constant_obs_h_list.append(
                    tf.reduce_max(tf.abs(self.grad_res_h[i])) / tf.reduce_mean(tf.abs(self.grad_obs_h[i])))

        for i in range(len(layers2) - 1):
            self.adpative_constant_bcs_u_list.append(
                tf.reduce_max(tf.abs(self.grad_res_u[i])) / tf.reduce_mean(tf.abs(self.grad_bcs_u[i])))
            if self.useObs:
                self.adpative_constant_obs_u_list.append(
                    tf.reduce_max(tf.abs(self.grad_res_u[i])) / tf.reduce_mean(tf.abs(self.grad_obs_u[i])))
    ...
    def initialize_NN(self, layers):
        ...
        W1 = self.xavier_init(size=[2 * layers[-2], layers[-1]])
        W2 = self.xavier_init(size=[2 * layers[-2], layers[-1]])
    ...
    def waveact(self,input):
        #return 1/tf.sqrt(2)*(tf.sin(input)+tf.cos(input))
        return tf.sin(input)+tf.cos(input)

    def neural_net_h(self, X, weights, biases):
        num_layers = len(self.layers1)
        ...
        H1_t = tf.concat([tf.sin(tf.matmul(t, self.Wh1_t)),
                          tf.cos(tf.matmul(t, self.Wh1_t))], 1)

        H2_t = tf.concat([tf.sin(tf.matmul(t, self.Wh2_t)),
                          tf.cos(tf.matmul(t, self.Wh2_t))], 1)

        H1_x = tf.concat([tf.matmul(x, self.Wh1_x),
                          tf.matmul(x, self.Wh1_x)], 1)
        for l in range(0, num_layers - 2):
            W1, W2 = weights[l]
            b = biases[l]
            # H1_t part
            H1_t1 = tf.add(tf.matmul(H1_t, W1), b)
            H1_t2 = tf.matmul(H1_t, W2)
            H1_t=tf.tanh(tf.add(H1_t1 * H1_t2, H1_t1))


            # H2_t part
            H2_t1 = tf.add(tf.matmul(H2_t, W1), b)
            H2_t2 = tf.matmul(H2_t, W2)
            H2_t = tf.tanh(tf.add(H2_t1 * H2_t2, H2_t1))


            # H1_x part
            H1_x1 = tf.add(tf.matmul(H1_x, W1), b)
            H1_x2 = tf.matmul(H1_x, W2)
            H1_x = tf.tanh(tf.add(H1_x1 * H1_x2, H1_x1))
    ...
    def neural_net_u(self, X, weights, biases):
        ...
        #     # Fourier feature encodings
        H1_t = tf.concat([tf.sin(tf.matmul(t, self.Wu1_t)),
                          tf.cos(tf.matmul(t, self.Wu1_t))], 1)

        H2_t = tf.concat([tf.sin(tf.matmul(t, self.Wu2_t)),
                          tf.cos(tf.matmul(t, self.Wu2_t))], 1)

        H1_x = tf.concat([tf.matmul(x, self.Wu1_x),
                          tf.matmul(x, self.Wu1_x)], 1)
    ...
    def fwd_gradients2(self, U, x):
        g = tf.gradients(U, x, grad_ys=self.dummy_tf2)[0]
        return tf.gradients(g, self.dummy_tf2)[0]
    ...
    def net_uh(self, x, t):
        ...
        u = self.neural_net_u(X, self.weights2, self.biases2)
        h = self.neural_net_h(X, self.weights1, self.biases1)
    ...
    def net_f(self, x_f, t_f):
        ...
        u = self.neural_net_u(X_f, self.weights2, self.biases2)
        h = self.neural_net_h(X_f, self.weights1, self.biases1)
    ...
    def fun_r_momentum(self, u, h, u_t, u_x, h_x,alpha):
        ...
        return (u_t + u*u_x + 9.81*h_x +
                (1-alpha)*9.81* ( n*n*tf.abs(u)*u / tf.pow( tf.square(R), 2./3)   - self.S) + alpha*(n*u*tf.abs(u)/2/self.D))

    def callback_obs(self, loss, loss_f_c, loss_f_m, loss_BC_u, loss_BC_h, loss_IC_h, loss_obs_u, loss_obs_h):
        self.count = self.count+1
        print('{} th iterations, Loss: {:.3e}, Loss_f_c: {:.3e}, Loss_f_m: {:.3e}, Loss_obs_u: {:.3e}, Loss_obs_h: {:.3e}'
    ...
    def train(self, num_epochs):
        ...
        tf_dict = {self.x_h_IC_tf: self.x_h_IC, self.t_h_IC_tf: self.t_h_IC, self.h_IC_tf: self.h_IC, 
                   ...
                   self.dummy_tf1: np.ones((self.t_f.shape[0], self.layers1[-1])),
                   self.dummy_tf2: np.ones((self.t_f.shape[0], self.layers2[-1])),
